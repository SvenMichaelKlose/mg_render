...
static void
our_getline (char *dest, int n, FILE * stream)
{
    int c;
    int i;
...
static FILE *
examine (const char *name, const char *extension, const char *header)
{
    FILE *stream;
    char *file;
...
image_load (const char *name)
{
    ...
    for (y = 0; y < image->rows; y++)
        for (x = 0; x < image->cols; x++)
            switch (image->type) {
            case BYTE_IMG:
                b_pixel (image, x, y) = (BYTE) fgetc (stream);
                break;
            case RGB_IMG:
                rgb_pixel (image, x, y).r = (BYTE) fgetc (stream);
                rgb_pixel (image, x, y).g = (BYTE) fgetc (stream);
                rgb_pixel (image, x, y).b = (BYTE) fgetc (stream);
                break;
            case FLOAT_IMG:
                fread (&f_pixel (image, x, y), sizeof (FLOAT), 1, stream);
                break;
            default:
                break;
            }
...
void
image_save (IMAGE * image, const char *name)
{
    FILE *stream;
    char *file, *header;
    unsigned int x, y;
    ...
    for (y = 0; y < image->rows; y++)
        for (x = 0; x < image->cols; x++)
            switch (image->type) {
            case BYTE_IMG:
                fputc (b_pixel (image, x, y), stream);
                break;
            case RGB_IMG:
                fputc (rgb_pixel (image, x, y).r, stream);
                fputc (rgb_pixel (image, x, y).g, stream);
                fputc (rgb_pixel (image, x, y).b, stream);
                break;
            case FLOAT_IMG:
                fwrite (&f_pixel (image, x, y), sizeof (FLOAT), 1, stream);
                break;
            default:
                break;
            }
